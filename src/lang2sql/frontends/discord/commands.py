...
from ...semantic.catalog import DimensionDisclosureTier
...
class CommandHandlers:
    ...
    async def semantic_candidates(
        ...
        if needle:
            candidates = [
                ...
                if needle
                in " ".join(
                    (item.id, item.label, item.classification_evidence)
                ).lower()
            ]
        if not candidates:
        ...
        if page > page_count:
            return OutboundMessage(
                text=f"페이지 범위는 1~{page_count}입니다."
            )
        ...
        for candidate in shown:
            ...
            if candidate.raw_output_allowed:
                revoke_token = (
                    self._concierge.semantic.issue_dimension_action_token(
                        identity.kv_scope,
                        candidate.id,
                        "dimension_revoke",
                        expected_catalog=catalog_snapshot,
                ...
                revoke_text = (
                    f" / revoke_token: {_display_token(revoke_token)}"
                )
            lines.append(
    ...
    async def semantic_reviews(
        ...
        if identity.guild_id and not identity.is_admin:
            return OutboundMessage(text="❌ 의미 검토 대기열은 관리자만 볼 수 있습니다.")
            ...
            queue = [
                ...
                in " ".join(
                    (
                        ...
                        pair[1].requester_id,
                        pair[1].metric_id,
                        pair[1].metric_phrase,
                        pair[1].review_kind,
                    )
                ).lower()
            ]
        if not queue:
            return OutboundMessage(text="현재 연결에 확인 대기 중인 의미 검토가 없습니다.")
        page_size = 20
        ...
        for _review_scope, pending in queue[start : start + page_size]:
            lines.append(
                ...
                f"{_display(pending.review_kind)} / 지표: "
                f"{_display(pending.metric_id)} / 표현: "
                f"{_display(pending.metric_phrase)} / 선택: "
    ...
    async def semantic_public_data(
        ...
        if identity.guild_id and not identity.is_admin:
            return OutboundMessage(text="❌ 공개 데이터 범위 변경은 관리자만 할 수 있습니다.")
        if not confirm:
    ...
    async def ingest(
        ...
        result = await ctx.tools.dispatch_direct(
            "ingest_doc", args, ctx, "cmd:ingest"
        )
        return OutboundMessage(text=result.content)
