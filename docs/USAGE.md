...
| 기능 | Catalog 없는 legacy mode | Catalog 있는 연결 즉시 의미 준비형 질의 모드 |
|---|---|---|
...
차단 사유를 무시하고 `run_sql`로 우회하는 것은 연결 즉시 의미 준비형 질의 모드의 복구
방법이 아니다.

## 기타 기존 명령

`/ingest`, `/confirm_ingest`, `/term_custom`, `/remember`, `/audit_me`는 기존
기능으로 남아 있다. `/enrich`와 `/org_setup`의 raw-value sampling은 연결 즉시
의미 준비형 질의가 활성화된 DB에서는 의도적으로 비활성화된다.
